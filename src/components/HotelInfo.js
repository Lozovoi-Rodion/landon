...
const HotelInfo = () => {
  ...
  const loadAccessibilityData = useCallback(async () => {
    ...
    const resp = await fetch(
      "https://n4j2uzyea4.execute-api.us-east-1.amazonaws.com/Production/accessibilities"
    );
  ...
  const loadServicesData = useCallback(async () => {
    // Query the API Gateway
    const resp = await fetch(
      "https://n4j2uzyea4.execute-api.us-east-1.amazonaws.com/Production/services"
    );
    let jsonData = await resp.json();
  ...
  useEffect(() => {
    ...
    loadServicesData();
  }, [loadAccessibilityData, loadServicesData]);
